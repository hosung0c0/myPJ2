...
function loadFn() {
    ...
    abtn[1].onclick = () => {
        ...
        setTimeout(() => {
            // 1. 맨앞li 잘라서 맨뒤로 이동!
            banner.appendChild(banner.querySelectorAll("li")[0]);
            // 2. left값 0으로 초기화!
            banner.style.left = "0";
            /* 3. 트랜지션 없애기 */
    ...
    abtn[0].onclick = () => {
        ...
        if (prot) return; // 나가!
        prot = 1; // 잠금!
        setTimeout(() => (prot = 0), 1800);
        ////////////////////////////

        // 1. 맨뒤요소를 잘라서 맨앞으로 이동한다!
        // 대상: banner변수 -> ul#banner
        // 사용메서드: insertBefore(넣을놈,넣을놈전놈)
        // 현재 li자식요소 수집하기!
        let cli = banner.querySelectorAll("li");
        banner.insertBefore(cli[cli.length - 1], cli[0]);
    ...
    // 대상: #banner li -> 여기에 속성을 넣는다!
    // 사용메서드 : forEach((요소,순번)=>{코드})
    // 사용메서드 : setAttribute(속성명,값)
    banner.querySelectorAll("li").forEach((ele, idx) => {
